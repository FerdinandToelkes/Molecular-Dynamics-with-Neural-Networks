...
def convert_trajectory_to_ase(coords_traj: np.ndarray, energy_traj: np.ndarray, forces_traj: np.ndarray, atomic_numbers: list) -> tuple:
    """
    ...
    atoms_list = []
    property_list = []
    for positions, energies, forces in zip(coords_traj, energy_traj, forces_traj):
...
def main(trajectory_dir: str):
    ...
    if not os.path.exists(traj_path) or not os.path.exists(energy_path) or not os.path.exists(grads_path):
        ...
        raise FileNotFoundError(f"One or more files do not exist in the specified directory: {data_prefix}\n"
                                "Try running log2x > coordinates.xyz, log2egy > energies.txt and the log2grad.py script in the same directory.")

    logger.debug("Extracting data from .xyz and .txt files")
    coords_traj, number_of_atoms = extract_data_from_xyz(traj_path, extra_lines=2)
