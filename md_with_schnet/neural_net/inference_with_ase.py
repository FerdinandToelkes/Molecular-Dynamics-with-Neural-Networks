...
"""
screen -dmS inference_xtb sh -c 'python -m md_with_schnet.neural_net.inference_with_ase --trajectory_dir MOTOR_MD_XTB/T300_1 ; exec bash'
"""
...
def main(trajectory_dir: str, model_dir: str, md_steps: int, seed: int, num_workers: int):
    ...
    xtb_dir = os.path.join(md_workdir, "xtb")
    os.makedirs(xtb_dir, exist_ok=True)  # Ensure the directory exists
    ...
    structure = datamodule.train_dataset[0]
    ...
    if isinstance(velocities_au, np.ndarray):
        velocities = velocities_au.astype(np.float64)
    ...
    # Set provided velocities
    # Conversion factors
    bohr_to_angstrom = units.Bohr  # 1 Bohr = 0.52917721067 Å
    aut_to_fs = 0.02418884326505
    print(f"units.Bohr: {units.Bohr}, units._aut: {units._aut}")
    velocities_ase = velocities_au * bohr_to_angstrom / aut_to_fs  # Convert from Bohr/fs to Å/fs
    ...
    dyn_xtb = ASEVelocityVerlet(
        ...
        trajectory=f'{md_workdir}/xtb_traj.traj', 
        logfile=f'{md_workdir}/xtb_md.log',
        loginterval=1
    )
    ...
    dyn_nn = ASEVelocityVerlet(
        atoms_nn, 
        timestep=0.5 * units.fs, 
        trajectory=f'{md_workdir}/nn_traj.traj', 
        logfile=f'{md_workdir}/nn_md.log',
        loginterval=1
    )
