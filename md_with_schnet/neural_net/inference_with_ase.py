...
from md_with_schnet.units import convert_time, convert_velocities
...
def parse_args() -> dict:
    ...
    parser.add_argument("-mdir", "--model_dir", type=str, default="MOTOR_MD_XTB_T300_1_ang_kcal_mol_epochs_1000_bs_100_lr_0.0001_seed_42", help="Directory of the trained model (default: MOTOR_MD_XTB_T300_1_ang_kcal_mol_epochs_1000_bs_100_lr_0.0001_seed_42)")
...
def main(trajectory_dir: str, units: str, model_dir: str, md_steps: int, time_step: float, fold: int, seed: int, num_workers: int):
    ...
    path_to_db2 = os.path.join(data_prefix, trajectory_dir, f"md_trajectory_bohr_hartree_aut.db")
    ...
    xtb_datamodule2 = setup_datamodule(data_cfg=cfg.org_data, datapath=path_to_db2, split_file=split_file)
    nn_datamodule = setup_datamodule(data_cfg=cfg.data, datapath=path_to_db, split_file=split_file)

    # iterate over xtb and xtb2 and check if they are the same
    for i in range(len(xtb_datamodule.val_dataset)):
        xtb_structure = xtb_datamodule.val_dataset[i]
        xtb_structure2 = xtb_datamodule2.val_dataset[i]
        if not torch.allclose(xtb_structure["_positions"], xtb_structure2["_positions"]):
            logger.warning(f"Structures in xtb and xtb2 datasets are not the same for index {i}.")
        if not torch.allclose(xtb_structure["_atomic_numbers"], xtb_structure2["_atomic_numbers"]):
            logger.warning(f"Atomic numbers in xtb and xtb2 datasets are not the same for index {i}.")
        if not torch.allclose(xtb_structure["velocities"], xtb_structure2["velocities"]):
            logger.warning(f"Velocities in xtb and xtb2 datasets are not the same for index {i}.")
        if not torch.allclose(xtb_structure["forces"], xtb_structure2["forces"]):
            logger.warning(f"Forces in xtb and xtb2 datasets are not the same for index {i}.")
        if not torch.allclose(xtb_structure["energy"], xtb_structure2["energy"]):
            logger.warning(f"Energy in xtb and xtb2 datasets are not the same for index {i}.")
