...
from excited_state_md.preprocessing.utils import prepare_last_excited_cycles
...
"""
python -m excited_state_md.preprocessing.prepare_tddft_data  --num_atoms 48 --position_unit bohr --energy_unit hartree --time_unit aut --nr_of_dirs 3
"""
...
def parse_args() -> dict:
    ...
    """
    parser = argparse.ArgumentParser(description="Prepare TDDFT data for usage in SPaiNN")
    ...
    parser.add_argument("--nr_of_dirs", type=int, default=-1, help="Number of directories to process. If -1, all directories will be processed (default: -1)")
...
def main(target_dir: str, nr_of_dirs: int, computed_cycles: int, num_atoms: int, position_unit: str, energy_unit: str, time_unit: str, seed: int):
    """
    ...
    Args:
        ...
        nr_of_dirs (int): Number of directories to process. If -1, all directories will be processed (default: -1).
    ...
    geo_dirs = list(geo_dirs_with_last_excited_cycles.keys())
    np.random.shuffle(geo_dirs)  
    if nr_of_dirs > 0 and nr_of_dirs < len(geo_dirs):
        geo_dirs = geo_dirs[:nr_of_dirs]
    else:
        nr_of_dirs = len(geo_dirs)

    # setup paths for the new dataset    
    file_name = f"md_trajectory_{position_unit}_{energy_unit.replace('/', '_per_')}_{time_unit}_{nr_of_dirs}_dirs_used"
