...
"""
python3 -m excited_state_md.preprocessing.extract_s0_s1_gradients --target_dir PREPARE_12
"""

# Note: sadly the data is messy :(. The ground state gradients were originally not computed when in the excited S1 state. Now, the gradients for the excited S1 state have been computed, but only for the first 1461 cycles.

# (*) The s0 and s1 gradients are scattered across the gradient, gradient_ex and s0_gradient files. The gradient file contains the gradients of the active state, the gradient_ex file contains the s1 gradients starting from the last excited cycle, and the s0_gradient file contains the gradients of the ground state at least up to the last excited cycle. The s1 gradients are extracted from the gradient file until the last excited cycle and from the gradient_ex file from the last excited cycle to the end of the trajectory. The s0 gradients are extracted from the s0_gradient file until the last excited cycle and the rest is taken from the gradient file. Unfortunately, this is a bit Lionel ...
...
def parse_args() -> dict:
    ...
    """
    parser = argparse.ArgumentParser(description="Extract the gradients belonging to the ground state and excited energy for all GEO directories.")
...
def extract_state_gradients(output_path: str, first_gradient_path: str, second_gradient_path: str, last_excited_cycle: int, 
    ...
    Args:
        ...
        first_gradient_path (str): Path for cycles 1 to last_excited_cycle.
        second_gradient_path (str): Path for cycles last_excited_cycle+1 to total_cycles.
        last_excited_cycle (int): Last excited cycle index.
    ...
    with open(output_path, "w") as f:
        f.write(f"# Extracted {state_label} gradients from {first_gradient_path} (cycles 1-{last_excited_cycle})\n")
        f.write(f"# and {second_gradient_path} (cycles {last_excited_cycle+1}-{total_cycles})\n")
        f.write("# All properties are in atomic units\n")

    # Append extracted gradients
    os.system(f"bash {command_path} {first_gradient_path} 1 {last_excited_cycle} {time_step} >> {output_path}")
    os.system(f"bash {command_path} {second_gradient_path} {last_excited_cycle+1} {total_cycles} {time_step} >> {output_path}")
...
def main(target_dir: str, computed_cycles: int, total_cycles: int, time_step: int):
    ...
    command_path = os.path.expanduser(f'~/whk/code/excited_state_md/preprocessing/extract_gradients.sh')
    ...
    for geo_dir, last_excited_cycle in tqdm(last_excited_cycles.items(), desc="Extracting gradients"):
        logger.info(f"Processing {geo_dir} with last excited cycle {last_excited_cycle}")
        ...
        extract_state_gradients(
            ...
            last_excited_cycle=last_excited_cycle,
        ...
        logger.info(f"Extracted gradients for {geo_dir} with last excited cycle {last_excited_cycle} into {s0_output_path} and {s1_output_path}")
