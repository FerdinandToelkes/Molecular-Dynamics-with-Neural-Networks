...
"""
python3 -m excited_state_md.preprocessing.get_last_ex_cycle_of_valid_trajs --target_dir PREPARE_12
"""
...
def parse_args() -> dict:
    ...
    """
    parser = argparse.ArgumentParser(description="Extract hop cycle (last excited cycle) of valid trajectories.")
...
def get_last_excited_cycle(path: str, number_of_cycles: int) -> int:
    """
    Get the last excited cycle from the RUN.dat file. The working assumption is that the first configuration is in the excited state (1) and that there is only one hop.
    If the first configuration is not in the excited state, an error is raised.
    Args:
        path (str): Path to the RUN.dat file.
        number_of_cycles (int): The number of cycles.
    Raises:
        ValueError: If the first configuration is not in the excited state or if there are multiple hops.
    Returns:
        int: The last excited cycle.
    """
    excited_state = 1
    previous_state = excited_state 
    has_hopped = False
    last_excited_cycle = number_of_cycles
    with open(path, 'r') as f:
        for i, line in enumerate(f):
            active_state = int(line.strip())
            if i == 0 and active_state != excited_state:
                raise ValueError("The first configuration should be in the excited state.")
            ...
            if active_state != previous_state:
                ...
                has_hopped = True
                last_excited_cycle = i # again, assuming only one hop 
                logger.debug(f"Configuration changed from {previous_state} to {active_state} at line {i}.")
            previous_state = active_state
    logger.debug(f"Last excited cycle: {last_excited_cycle}")
    return last_excited_cycle

def save_last_excited_cycles_to_file(last_excited_cycles: dict, data_path: str):
    """
    Save the last excited cycles to a text file.
    Args:
        last_excited_cycles (dict): Dictionary containing the last excited cycles for each trajectory.
        data_path (str): Path to the directory where the text file will be saved.
    """
    # save the last excited cycles to a text file
    output_path = os.path.join(data_path, "last_excited_cycle_of_valid_trajectories.txt")
    with open(output_path, 'w') as f:
        for geo_dir, last_cycle in last_excited_cycles.items():
            f.write(f"{geo_dir}: {last_cycle}\n")
    logger.info(f"Last excited cycles saved to {output_path}")

def main(target_dir: str, expected_cycles: int):
    """
    Main function to extract the last excited cycle of valid trajectories.
    Args:
    ...
    for geo_dir in geo_dirs:
        ...
        last_excited_cycle = get_last_excited_cycle(path, number_of_cycles)
        last_excited_cycles[geo_dir] = last_excited_cycle
    
    # save the last excited cycles to a text file
    logger.info(f"Found {len(last_excited_cycles)} valid trajectories with last excited cycles: {last_excited_cycles}")
    save_last_excited_cycles_to_file(last_excited_cycles, data_path)
