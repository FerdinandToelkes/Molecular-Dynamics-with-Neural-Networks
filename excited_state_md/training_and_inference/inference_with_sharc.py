...
"""
screen -dmS inference_xtb sh -c 'python -m excited_state_md.training_and_inference.inference_with_sharc --sharc_dir PREPARE_12/GEO_100000; exec bash'
"""
...
def parse_args() -> dict:
    ...
    parser.add_argument("--sharc_dir", type=str, default="PREPARE_12/GEO_100000", help="Directory containing the trajectory data generated by Turbomole (default: PREPARE_12/GEO_100000)")
...
def main(trajectory_dir: str, sharc_dir: str, units: str, model_dir: str, md_steps: int, time_step: float, fold: int, seed: int, num_workers: int):
    data_prefix = set_data_prefix()
    print(f"Data prefix: {data_prefix}")
    
    models = ["./excited_state_md/training_and_inference/runs/bohr_hartree_aut/PREPARE_12_spainn_datasets/epochs_200_bs_16_lr_0.0001_flw_0.9_elw_0.05_nlw_0.05_seed_42/best_model"]
    th = None # for active learning, e.g., {'energy': 0.004}
    # symbols of sample molecule
    atoms = 'O' + 'C' * 27 + 'H' * 20 
    nn = SHARC_NN(
        modelpath=models, # path to NN models
        atoms=atoms, 
        n_states={'n_singlets': 2, 'n_triplets': 0}, # dict of state numbers
        thresholds=th,
    ...
    # change working directory to the sharc directory
    path_to_sharc = os.path.join(data_prefix, sharc_dir, "test/sharc_trajectory")
    os.chdir(path_to_sharc)
