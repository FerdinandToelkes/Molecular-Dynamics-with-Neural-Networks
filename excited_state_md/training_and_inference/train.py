...
# Enable faulthandler to get a stack trace on crashes
import faulthandler
faulthandler.enable()
...
"""
screen -dmS tddft_train sh -c 'python -m excited_state_md.training_and_inference.train --trajectory_dir PREPARE_12/spainn_datasets --units angstrom_kcal_per_mol_fs -e 200 -flw 0.98 -elw 0.01 -nlw 0.01 -bs 16 --nacs_key smooth_nacs --nr_of_dirs 3; exec bash'
"""
...
def parse_args() -> dict:
    ...
    parser.add_argument("--nr_of_dirs", type=int, default=3, help="Number of directories to process. (default: 3)")
    ...
    parser.add_argument("--nacs_key", type=str, default="smooth_nacs", choices=["smooth_nacs", "nacs"], help="Which NACS representation to use (default: smooth_nacs).")
    # training setup
...
def get_data_paths(data_prefix: str, trajectory_dir: str, nr_of_dirs: int, fold: int, units: str) -> tuple:
    """ 
    ...
    Args:
        ...
        nr_of_dirs (int): Number of directories used in the dataset.
    ...
    path_to_db = os.path.join(path_to_traj_dir, f"md_trajectory_{units}_{nr_of_dirs}_dirs_used.db")
    path_to_stats = os.path.join(path_to_traj_dir, f"means_stds_fold_{units}_{fold}.pt")
    if not os.path.exists(path_to_db):
        raise FileNotFoundError(f"Database file does not exist at {path_to_db}. Please run the prepare_tddft_data.py script first.")
    return path_to_db, path_to_stats


def set_run_path(trajectory_dir: str, nr_of_dirs: int, units: str, nacs_key: str, num_epochs: int, batch_size: int, learning_rate: float, 
                 forces_loss_weight: float, energy_loss_weight: float, nacs_loss_weight: float, seed: int) -> str:
    """ 
    Set the run path based on the trajectory directory.
    Args:
        trajectory_dir (str): Directory containing the trajectory data.
        units (str): Units for the input data.
        nacs_key (str): Key for the NACS representation.
    ...
    run_name = f"{nacs_key}_{nr_of_dirs}_dirs_used_epochs_{num_epochs}_bs_{batch_size}_lr_{learning_rate}_flw_{forces_loss_weight}_elw_{energy_loss_weight}_nlw_{nacs_loss_weight}_seed_{seed}"
    traj_dir = trajectory_dir.replace("/", "_")
    run_path = os.path.join(current_dir, "runs", units, traj_dir, run_name)
    return run_path

def update_config(cfg: DictConfig, run_path: str, ase_units: dict, nacs_key: str, batch_size: int, num_epochs: int, learning_rate: float, forces_loss_weight: float, energy_loss_weight: float, nacs_loss_weight: float, num_workers: int, path_to_stats: str) -> DictConfig:
    """ 
    ...
    Args:
        ...
        ase_units (dict): Dictionary containing units compatible with ASE.
        nacs_key (str): Key for the NACS representation.
        batch_size (int): Batch size for training.
        num_epochs (int): Number of epochs for training.
        learning_rate (float): Learning rate for the optimizer.
...
def main(trajectory_dir: str, nr_of_dirs: int, units: str, nacs_key: str, batch_size: int, num_epochs: int, learning_rate: float, 
    ...
    path_to_db, path_to_stats = get_data_paths(data_prefix, trajectory_dir, nr_of_dirs, fold, units)
    ...
    # set run path
    run_path = set_run_path(trajectory_dir, nr_of_dirs, units, nacs_key, num_epochs, batch_size, learning_rate, 
                            forces_loss_weight, energy_loss_weight, nacs_loss_weight, seed)
    ...
    cfg = update_config(cfg, run_path, ase_units, nacs_key, batch_size, num_epochs, learning_rate,
